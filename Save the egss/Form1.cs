...
// Score apres el quel la vitesse va augmenter a 12
            if (score > 10)
            {
                speed = 12;
            }

        // le nombre au quel le jeu doit s'arreter
            if (missed > 5)
            {
                GameTimer.Stop();
                MessageBox.Show("Game Over!" + Environment.NewLine + "You have lost good Eggs!" + Environment.NewLine + "Click ok to retry");
                RestartGame();
            }
